...
% x=0;
% y=0;
% [width,height] = size(imread(imgnames));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(imgnames)
    ...
    classname=batchClassnamePair{2}(logInd);
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    folder_filename=strsplit(filename{1},'/');
    tmpfolder = folder_filename{1};
    tmpfilename = folder_filename{2};
    [height,width,~]=size(imread(['../data/CUB_200_2011/images/',tmpfolder,'/',tmpfilename]));
    name=classname;
    bndbox.xmin=int2str(1);
    bndbox.xmax=int2str(width);
    bndbox.ymin=int2str(1);
    bndbox.ymax=int2str(height);
